...
function renderRedisMemoryCard(memory, writeHealth, retentionHours) {
  const retentionText = `Tracks recorded continuously (not just while racing), kept ${retentionHours}h, then expire automatically - "npm run clear-base-logs" clears sooner`;
  if (!memory) {
...
function renderDashboard(s, rtkControlsEnabled) {
  ...
  const boatRows = boatIds
    .map((id) => {
      ...
      // base-uploads (see baseStation.js's uploadDirBaseline) - this is the
    ...
    ${
      ...
      <thead>
        <tr>
          ...
          <th title="Total files ever received from this boat, from a one-time scan of base-uploads at startup - includes previous sessions, not just this one">Files on disk (all-time)</th>
