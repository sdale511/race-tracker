...
| `LOG_RECEIVED_FIXES` | unset (on) | Base station only — set to `0` to silence the `[base] boat=... lat,lon ...` console line printed for every single radio frame received from every boat (a full fleet at a normal GPS rate scrolls fast). Only the console echo is gated - the fix is still written to this base's own CSV log, still recorded to Redis, and lap/on-grid/mark-rounding/foul detection all still run exactly as before |
